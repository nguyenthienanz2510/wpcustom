	...
	public static function import_customizer_options( $data ) {
		...
		if ( ! is_array( $data ) || ! isset( $data['mods'] ) ) {
			return new WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'copenhagen' )
			);
		}
		...
		if ( isset( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new MBF_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
		...
		foreach ( $mods as $key => $val ) {
			if ( self::customizer_is_image_url( $val ) ) {
				...
				self::customizer_sideload_image(
					str_replace( array( '.jpg', '.jpeg', '.png', '.gif' ),
						array( '@2x.jpg', '@2x.jpeg', '@2x.png', '@2x.gif' ),
						$val
					)
				);

...
class MBF_Manager_Import {
	...
	public function ajax_import_plugin() {
		...
		if ( ! isset( $_POST['plugin_slug'] ) || ! sanitize_text_field( $_POST['plugin_slug'] ) ) {
			wp_send_json_error( esc_html__( 'Unknown slug in a plugin.', 'copenhagen' ) );
			wp_die();
		}

		if ( ! isset( $_POST['plugin_path'] ) || ! sanitize_text_field( $_POST['plugin_path'] ) ) {
			wp_send_json_error( esc_html__( 'Unknown path in a plugin.', 'copenhagen' ) );
			wp_die();
		}
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( esc_html__( 'Insufficient permissions to install the plugin.', 'copenhagen' ) );
			wp_die();
		}
		...
		wp_send_json_error( esc_html__( 'Failed to initialize or activate importer plugin.', 'copenhagen' ) );
	...
	public function ajax_import_contents() {
		...
		if ( ! isset( $_POST['url'] ) || ! sanitize_text_field( $_POST['url'] ) ) {
			wp_send_json_error( esc_html__( 'The url address of the demo content is not specified.', 'copenhagen' ) );
			wp_die();
		}
		...
		if ( ! $xml_file_path ) {
			...
			if ( ! current_user_can( 'edit_theme_options' ) ) {
				wp_send_json_error( esc_html__( 'You are not permitted to import contents.', 'copenhagen' ) );
				wp_die();
			}

			if ( ! isset( $file_url ) ) {
				wp_send_json_error( esc_html__( 'No XML file specified.', 'copenhagen' ) );
				wp_die();
			}
	...
	public function ajax_import_customizer() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'You are not permitted to import customizer.', 'copenhagen' ) );
			wp_die();
		}

		if ( ! isset( $file_url ) ) {
			wp_send_json_error( esc_html__( 'No customizer JSON file specified.', 'copenhagen' ) );
			wp_die();
		}
	...
	public function ajax_import_widgets() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'You are not permitted to import widgets.', 'copenhagen' ) );
			wp_die();
		}

		if ( ! isset( $file_url ) ) {
			wp_send_json_error( esc_html__( 'No widgets WIE file specified.', 'copenhagen' ) );
			wp_die();
		}
	...
	public function ajax_import_options() {
		...
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( esc_html__( 'You are not permitted to import options.', 'copenhagen' ) );
			wp_die();
		}

		if ( ! isset( $file_url ) ) {
			wp_send_json_error( esc_html__( 'No options JSON file specified.', 'copenhagen' ) );
			wp_die();
		}
